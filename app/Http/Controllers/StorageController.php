...
//            Log::info('validate success');
            if (Input::file('photo')->isValid()){
                ...
                $extension = Input::file('photo')->getClientOriginalExtension(); // 获取文件扩展名
                $fileName = rand(11111, 99999).time().".".$extension;
                Input::file('photo')->move($filePath, $fileName); // uploading file to given path
    ...
    public function upload(Request $request)
    {
        ...
        if ($validator->fails()){
            ...
            if (Input::file('photo')->isValid()){
                ...
                $im->save($path, $quality);
                Log::info('成功上传一张图片, 路径为：'.$path);
                return ['errno'=>0, 'data'=>[image_url('/uploads/'.date('Y-m').'/'.$fileName)]];
